...
class Snake:
    ...
    #Implementa uma interface de controle para um agente externo
...
#Classe Game - Implementa uma interface para o jogo
class Game():

    def __init__(self, visualization = 'train'):

        pygame.init()
        self.display = None #Armazena a superfície da janela

        self.game_state = 0 #Armazena o estado do jogo

        self.snake = Snake(self.display, 5, 5, sleep_time = 2000) #Instancia um objeto da classe Snake
        
        self.canvas = Canvas(self.display) #Instancia um objeto da classe Canvas

        self.apple = Food(self.display) #Instancia um objeto da classe Food
        self.apple.respawn(self.snake.body) #Gera a posição inicial da maçã

        self.score = Score(self.display) #Instancia um objeto da classe Score

        self.visualization = visualization #Inicializa o modo de visualização do jogo
                                           #train: aumenta a velocidade do jogo para acelerar o treinamento
                                           #test: deixa a velocidade do jogo no tempo normal para facilitar a visualização    
    
    #Quando invocdao, realiza uma ação no jogo e retorna o estado do jogo após a execução desta ação
    def game_step(self, action):
        ...
        self.snake.move()
        self.game_state = check_collisions(self.snake, self.apple, self.score)
        

        return {'gstate': self.game_state, 'food_pos':self.apple.pos, 
                'body':self.snake.body, 'dir':self.snake.direction, 'score':self.score.score}
    

    #Renderiza o jogo graficamente
    def reder(self):
        #Caso não haja uma janela aberta para a visualização do jogo, cria uma janela
        if self.display == None: 
            pygame.display.set_caption('SNAKE')
            self.display = pygame.display.set_mode((WIDTH,HEIGHT))
            self.snake.surface = self.display
            self.canvas.surface = self.display
            self.apple.surface = self.display
            self.score.surface = self.display
            
        draw_surfaces(self.canvas, self.score, self.apple, self.snake) #Desenha todos os objetos
        
        #Reduz a velocidade do jogo caso a visualizaçãõ seja do tipo 'test'
        if self.visualization == 'test':
            pygame.time.wait(100)

        pygame.display.update() #Atualiza a janela

    #Retorna o estao atual do jogo
    def observe(self):
        return {'gstate': self.game_state, 'food_pos':self.apple.pos, 
                'body':self.snake.body, 'dir':self.snake.direction, 'score':self.score.score}
...
#Implementa o controle do fluxo do jogo caso este esteja sendo jogado por um humano
